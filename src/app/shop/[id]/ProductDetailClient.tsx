...
export default function ProductDetailClient({ product }: { product: Product }) {
  ...
  const shareText = `Check out ${product.name} on Yemnest!`;
          ...
          <div className="reveal-el flex flex-col pt-4">
            ...
            <div className="flex items-center justify-center gap-6 text-[10px] uppercase tracking-widest text-zinc-400">
              <button 
                ...
                className={`flex items-center gap-2 transition-colors ${wishlist.some(item => item.id === product.id) ? 'text-red-500 hover:text-red-600' : 'hover:text-zinc-900'}`}
              >
              ...
              <div className="relative" ref={shareMenuRef}>
                ...
                {showShareMenu && (
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-white border border-zinc-200 shadow-xl rounded-md overflow-hidden z-50 p-1 flex flex-col gap-1 text-[11px]">
                    <a 
                      href={`https://api.whatsapp.com/send?text=${encodeURIComponent(shareText + " " + shareUrl)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 px-3 py-2 hover:bg-zinc-50 transition-colors text-zinc-700"
                    >
                    ...
                    <button 
                      ...
                      className="flex items-center gap-2 px-3 py-2 hover:bg-zinc-50 transition-colors text-zinc-700 text-left w-full"
                    >
                      <svg className="w-4 h-4 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                      {copiedLink ? "Copied!" : "Copy Link (Insta, etc.)"}
                    </button>
                  </div>
                )}
              </div>
